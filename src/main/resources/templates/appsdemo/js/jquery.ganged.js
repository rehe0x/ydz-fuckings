;(function($){
	...
	function handleData(datas){
		...
		var tempCities = [];
		var tempAreas = [];
		var reProvince=/^[0-9]{4}0{8}$/;//省的格式:前2位为'01~99'，后4位为'0000'  例：010000
		var reCity=/^[0-9]{8}0{4}$/;//城市的格式:前2位为'01~99',中间2位'01~99',后2位为'00' 例:010100
		var reArea=/^[0-9]{12}$/;//区的格式:前2位为'01~99',中间2位'01~99',后2位为'01~99' 例:010101
		...
		for(var i=0; i<tempProvinces.length; i++){
			var pId = tempProvinces[i].id.substring(0,4);
			for(var j=0; j<tempCities.length; j++){
				if(tempCities[j].id.substring(0,4) == pId){
		...
		for(var i=0; i<tempCities.length; i++){
			var cId = tempCities[i].id.substring(0,8);
			for(var j=0; j<tempAreas.length; j++){
				if(tempAreas[j].id.substring(0,8) == cId){
